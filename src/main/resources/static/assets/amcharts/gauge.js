AmCharts.GaugeAxis = AmCharts.Class({
    construct: function (a) {
        ...
        var m = a.container, q = this.tickColor,
            u = this.tickAlpha, C = this.tickThickness, D = f / h, G = g / D, h = this.minorTickLength,
            I = this.labelFrequency, s = this.radiusReal;
        this.inside || (s -= 15);
        ...
        for (n = 0; n < l; n++) {
            var v = c + n * f, r = p + n * g, w = k(y + s * Math.sin(r / 180 * Math.PI)),
                B = k(z - s * Math.cos(r / 180 * Math.PI)), x = k(y + (s - e) * Math.sin(r / 180 * Math.PI)),
                t = k(z - (s - e) * Math.cos(r / 180 * Math.PI)),
                w = AmCharts.line(m, [w, x], [B, t], q, u, C, 0, !1, !1, !0);
            b.push(w);
            w = -1;
            x = this.labelOffset;
            this.inside || (x = -x - e, w = 1);
            var B = Math.sin(r / 180 * Math.PI), t = Math.cos(r / 180 * Math.PI), B = y + (s - e - x) * B,
                x = z - (s - e - x) * t, E = this.fontSize;
...
AmCharts.AmAngularGauge = AmCharts.Class({
    inherits: AmCharts.AmChart, construct: function (a) {
        ...
        this.realHeight = c;
        var d = AmCharts.toCoordinate, f = d(this.marginLeft, b), h = d(this.marginRight, b),
            p = d(this.marginTop, c) + this.getTitleHeight(), k = d(this.marginBottom, c), e = d(this.radius, b, c),
            d = b - f - h, l = c - p - k + this.extraHeight;
        e || (e = Math.min(d, l) / 2);
        ...
        a.set = c.set();
        if (!a.hidden) {
            var d = a.axis, f = d.radiusReal, h = d.centerXReal, p = d.centerYReal, k = a.startWidth, e = a.endWidth,
            ...
            var d = Math.sin(b / 180 * Math.PI), f = Math.cos(b / 180 * Math.PI),
                m = Math.sin((b + 90) / 180 * Math.PI), u = Math.cos((b + 90) / 180 * Math.PI),
                c = AmCharts.polygon(c, [h - k / 2 * m + l * d, h + g * d - e / 2 * m, h + g * d + e / 2 * m, h + k / 2 * m + l * d], [p + k / 2 * u - l * f, p - g * f + e / 2 * u, p - g * f - e / 2 * u, p - k / 2 * u - l * f], a.color, a.alpha, 1, q, a.borderAlpha, void 0, !0);
            a.set.push(c);
